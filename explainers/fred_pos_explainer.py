...
class FRED_pos(Explainer):
    """
    FRED_pos explainer for text classification.

    Inherits from the base Explainer class and provides methods for explaining
    text predictions using LIME.
    """

    def __init__(self, name, class_names, model, corpus, args=None):
        """
        Initializes the FRED_pos explainer.
    ...
    def explain(self, example):
        """
        Explains a text instance using FRED_pos.

        Args:
            example (str): The text instance to be explained.

        Returns:
            object: The explanation object returned by FRED_pos.
        """
    ...
    def get_ranked_tokens(self, example, fredpos_exp):
        """
        Retrieves tokens ranked by importance from a FRED explanation.

        Args:
            example (str): The text instance.
            fredpos_exp (object): The FRED explanation object.
        ...
        ids_by_importance = [key for key, value in fredpos_exp.token_drops_ids.items() if value >= 0]
        return list(tokens[ids_by_importance]), ids_by_importance

    def get_top_features(self, example, fredpos_exp, k=10):
        """
        ...
        """
        return fredpos_exp.best, fredpos_exp.best_ids

    def get_tokens_importance(self, fredpos_exp):
        """
        ...
        """
        return fredpos_exp.token_drops
